...
error meant", and then after I understood it I asked "how to fix it."

## Problem 1: dependency resolution conflict (`uv add`)
...
`--locked` is an assertion, so "the lockfile is already consistent with
`pyproject.toml`, just install it, and error if that's not true." Because I
had added `tabulate` to `pyproject.toml` without relocking, the manifest
and `uv.lock` disagreed, so the assertion failed on purpose. This is the
behaviour you want in CI.

### Question 2: "how do I fix it?"

Claude said there are two different fixes.
